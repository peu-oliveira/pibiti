...
void ParamListGL::Render(int x, int y, bool shadow)
{
	...
	for (ParamIter p = params.begin(); p != params.end(); ++p)
		if (*p)
		{
			...
			{
				if (p == cur)
					glColor3fv(text_clr_sel); // text
				else
					glColor3fv(text_clr_unsel);
				...
				glVertex2f(x + bar_x, y);
				glVertex2f(x + bar_x + bar_w, y);
				glVertex2f(x + bar_x + bar_w, y + bar_h);
				glVertex2f(x + bar_x, y + bar_h);
				glEnd();

				if (p == cur)
					glColor4fv(bar_clr_insel); //  fill
				else
					glColor4fv(bar_clr_inner);
...
bool ParamListGL::Mouse(int x, int y, int button, int state)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
		if (x > bar_x - bar_dx && x < bar_x + bar_w + bar_dx)
		{
			...
			for (cur = params.begin(); cur != params.end() && i > 0; cur++, i--)
				; //cur = (ParamIter)&params.at(i);
			if (!*cur)
				cur = curTemp;
...
bool ParamListGL::Motion(int x, int y)
{
	...
	if (x < bar_x)
	{
		(*cur)->SetPercentage(0.0);
		ParamBase::Changed();
		return isLMBDown;
	}
	if (x > bar_x + bar_w)
	{
		(*cur)->SetPercentage(1.0);
		ParamBase::Changed();
		return isLMBDown;
	}

	(*cur)->SetPercentage((x - bar_x) / (float)bar_w);
	ParamBase::Changed();
	if ((y < start_y) || (y > (int)(start_y + (text_y * params.size()) - 1)))
		return isLMBDown;
	return true;
...
void ParamListGL::Special(int key, int /*x*/, int /*y*/)
{
	...
	case GLUT_KEY_UP:
		...
		break;
	case GLUT_KEY_RIGHT:
		GetCurrent()->Increment(t);
		ParamBase::Changed();
		break;
	case GLUT_KEY_LEFT:
		GetCurrent()->Decrement(t);
		ParamBase::Changed();
		break;
	case GLUT_KEY_HOME:
		GetCurrent()->Reset();
