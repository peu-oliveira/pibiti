...
void App::Motion(int x, int y)
{
	...
	if (bSliders)
		if (x < tabPosX[NumTabs - 1] + tabWidths[NumTabs - 1])
			...
			{
				for (int i = 0; i < nTabRow2; i++)
					if (x > tabPosX[i])
						mselTab = i;
			}												  // search
			else if (y >= yParList + 10 && y < yParList + 38) // 2nd
			{
				for (int i = nTabRow2; i < NumTabs; i++)
					if (x > tabPosX[i])
						mselTab = i;
			}
	...
	if (mselTab == -1 && buttons > 0)
		...
		break;
		case M_MOVE:
			p = (float3 *)&colliderPos;
			break;
		case M_ACCEL:
			p = &q.accPos[q.ca];
			break;
		case M_DYE:
			p = &dyePos; /*&q.params.dyePos;*/
			break;
		}
...
void App::updHue()
{
	int h = pParRend->m_nProg;
	psys->scn.params.iHue = h;
...
{
	if (pParRend->m_nProg)
		iHueType = (iHueType - 1 + HUE_ALL) % HUE_ALL;
	else
		iClrType = (iClrType - 1 + CLR_ALL) % CLR_ALL;
	updHue();
}
void App::nextClr()
{
	if (pParRend->m_nProg)
		iHueType = (iHueType + 1) % HUE_ALL;
	else
		iClrType = (iClrType + 1) % CLR_ALL;
	updHue();
}

void App::prevMode()
{
	mode = (Modes)(((int)(mode)-1 + M_ALL) % M_ALL);
	updTabVis();
}
void App::nextMode()
{
	mode = (Modes)(((int)(mode) + 1) % M_ALL);
	updTabVis();
}
...
void App::prevTab()
{
	do
	{
		curTab = (curTab - 1 + NumTabs) % NumTabs;
	} while (tabHidden[curTab] == 1);
	setCurL();
}
void App::nextTab()
{
...
void App::prevEmit()
{
	psys->scn.ce = (psys->scn.ce - 1 + NumEmit) % NumEmit;
	setCurL();
}
void App::nextEmit()
{
	psys->scn.ce = (psys->scn.ce + 1) % NumEmit;
	setCurL();
}
void App::prevAcc()
{
	psys->scn.ca = (psys->scn.ca - 1 + NumAcc) % NumAcc;
	setCurL();
}
void App::nextAcc()
{
	psys->scn.ca = (psys->scn.ca + 1) % NumAcc;
	setCurL();
}
...
void App::setCurL(bool norm)
{
	...
	{
	case 2:
		curParList = parLisE[psys->scn.ce];
		break;
	case 3:
		curParList = parLisA[psys->scn.ca];
		break;
	case 5:
		curParList = parLisV[pParRend->m_nProg];
		break;
	default:
		curParList = parLists[curTab];
		break;
	}
...
void App::updTabVis() // hide unused tabs
{
	...
	bool b = false;
	int i = 0; // search active emitters
	if (psys->scn.rain > 0)
		b = true;
	else if (mode == M_EMIT)
		b = true;
...
void App::togHue()
{
	pParRend->m_nProg = pParRend->m_nProg > 0 ? 0 : 1;
	updHue();
	setCurL(); /*updTabVis();*/
}
...
void App::KeyPressed(unsigned char k, int x, int y)
{
	...
	case 'd':
		...
		break;
	case 'D':
		psys->scn.params.dyeType = psys->scn.params.dyeType == 0 ? 1 : 0;
		ParamBase::Changed();
		break;
	...
	case '=':
	case '+':
		psys->scn.emit[psys->scn.ce].IncSize();
		break;
	case '-':
		psys->scn.emit[psys->scn.ce].DecSize();
		break;

	case '[':
		parLisE[psys->scn.ce]->GetParam("Velocity")->Decrement(1);
		break;
	case ']':
		parLisE[psys->scn.ce]->GetParam("Velocity")->Increment(1);
		break;
...
void App::KeySpecial(int k, int x, int y)
{
	...
	case GLUT_KEY_F4:
		if (shift)
			iTimes = (iTimes - 1 + NumTimM) % NumTimM;
		else
	...
	case GLUT_KEY_F5:
		psys->Reset(shift ? 1 : psys->scn.initType);
		fSimTime = 0.f;
		break; // type
...
void App::UpdateCamera()
{
	...
	if (fBackgr != foBackgr)
	{
		glClearColor(0.2 * fBackgr, 0.3 * fBackgr, 0.4 * fBackgr, 1.0);
		foBackgr = fBackgr;
	}
	...
	glRotatef(camRotLag.x, 1, 0, 0);
	glRotatef(camRotLag.y, 0, 1, 0);
...
void App::Mouse(int button, int state, int x, int y)
{
	...
	///  change tab  ****
	if (buttons == 1 && mselTab >= 0 && tabHidden[mselTab] == 0)
	{
		curTab = mselTab;
		setCurL();
	}
	...
	my = y;
	if (bSliders)
		curParList->Mouse(x, y, button, state);
}
