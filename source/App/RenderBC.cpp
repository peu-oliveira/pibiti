...
void App::DrawCollider()
{
	...
	if (par.rotType > 0)
	{
		...
		for (int i = 0; i < nRots; i++)
		{
			...
			{
#define drawRot                                                                          \
	float clr = float(y + 1) / float(sy + 1) * 0.5f, scl = mode == M_MOVE ? 0.8f : 0.5f; \
	glColor3f(clr / 4, scl + clr / 3, clr);                                              \
	glPushMatrix();                                                                      \
	glTranslatef(p0.x + pc.x, p0.y + pc.y, p0.z + pc.z);                                 \
	drawSphere(r);                                                                       \
	glPopMatrix();
			case 1: // pump rotor Z -
				for (int z = 1; z <= sz; z++)
					for (int x = 0; x <= sx; x++)
						for (int c = 0; c < cb; c++)
						{
							...
							for (int y = 0; y <= sy; y++)
							{
								float3 pc = make_float3((x - x2) * cs - z * sn, (x - x2) * sn + z * cs, sp * y);
								drawRot
							}
						}
				break;
				...
				for (int z = 1; z <= sz; z++)
					for (int x = 0; x <= sx; x++)
						for (int c = 0; c < cb; c++)
						{
							...
							for (int y = 0; y <= sy; y++)
							{
								float3 pc = make_float3((x - x2) * cs - z * sn, sp * y, (x - x2) * sn + z * cs);
								drawRot
							}
						}
				break;
	...
	{ //  collider
		...
		glPushMatrix();
		float4 pos = psys->scn.params.collPos;
		glTranslatef(pos.x, pos.y, pos.z);
		glutWireSphere(psys->scn.params.collR, 20, 10);
		glPopMatrix();
	}
	...
	for (int i = 0; i < NumEmit; i++)
	{
		...
		if (em.size > 0 || sel)
		{
			...
			glPushMatrix();
			glTranslatef(em.posLag.x, em.posLag.y, em.posLag.z);
			glRotatef(em.rotLag.y, 0, 1, 0);
			glRotatef(-em.rotLag.x, 1, 0, 0);
			float spc = psys->scn.spacing;
			int er = max(1, em.size), er2 = (em.size2 == 0) ? er : em.size2;
	...
	for (int i = 0; i < NumAcc; i++)
	{
		...
		case ACC_Box:
			glPushMatrix();
			glTranslatef(ac.pos.x, ac.pos.y, ac.pos.z);
		...
		case ACC_CylY:
			...
			drawCylY(1.f, -ac.size.y, ac.size.y);
			glPopMatrix();
			break;
		...
		case ACC_CylYsm:
			...
			drawCylY(ac.size.z, -ac.size.y, ac.size.y);
			glPopMatrix();
			break;
		}
	}
	...
	if (sel || p.dyeType > 0)
	{
		...
		glPushMatrix();
		glTranslatef(p.dyePos.x, p.dyePos.y, p.dyePos.z);
		switch (p.dyeType)
		{
		case 1:
			glScalef(p.dyeSize.x * 2, p.dyeSize.y * 2, p.dyeSize.z * 2);
			glutWireCube(1.f);
			break;
		case 2:
			drawSphere(p.dyeSize.y);
			break;
		}
		glPopMatrix();
	}
...
void App::DrawBounds()
{
	...
	{
		glBegin(GL_LINES);
		float m = -0.001f, s = 0.1f + m;
		glColor3f(1, 1, 1);
		V(minX + m, minY + m, minZ + m);
		glColor3f(1, 0, 0);
		V(minX + s, minY + m, minZ + m);
		glColor3f(1, 1, 1);
		V(minX + m, minY + m, minZ + m);
		glColor3f(0, 1, 0);
		V(minX + m, minY + s, minZ + m);
		glColor3f(1, 1, 1);
		V(minX + m, minY + m, minZ + m);
		glColor3f(0, 0, 1);
		V(minX + m, minY + m, minZ + s);
		glEnd();
	}
	...
	{
		float minY2 = minY;
		bool bWave = p->bndEffZ == BND_EFF_WAVE;
		if (bWave)
			minY2 += /*p->distBndSoft*/ -(maxZ - minZ) * -p->r2Angle /*+ p->particleR/2.f*/;
	...
	case BND_CYL_Y:
		drawCylY(maxX, minY, maxY);
		break;
	case BND_CYL_Z:
		drawCylZ(minY, minZ, maxZ);
		break;
