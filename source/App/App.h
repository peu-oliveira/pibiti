...
public:
	...
	static float4 colliderPos;
	static float modelView[16], inertia;
	...
	//  Info
	static enum Modes { M_VIEW = 0,
						M_MOVE,
						M_EMIT,
						M_ACCEL,
						M_DYE,
						M_ALL } mode;
	static bool bSliders, bText, bInfo, bHelp, bXyz, bScale;
	static int iTimes, WSizeX, WSizeY;
	;													  // bShow
	static bool bPaused, bWindowed, bVsyncOff, bShowInfo; // init opts

	static char s[256];
	static uint timer[NumTim];
	static float tm[NumTim], tdt, tdts, tm_si;
	static int timCnt, timAvgCnt;
	static float barsScale, fSimTime;
	...
	static char tabNames[NumTabs][20];
	static float3 tabClrs[NumTabs];
	static int tabWidths[NumTabs], tabPosX[NumTabs], tabHidden[NumTabs], curTab, yParList, mselTab, nTabRow2, nChg;
	static void updHue(), updBrCnt();
	static int iHueType, iClrType;
...
public:
	...
	static void drawCircleY(float r, float y), drawCircleZ(float r, float z), drawCircleX(float r, float x);
	static float aq;
