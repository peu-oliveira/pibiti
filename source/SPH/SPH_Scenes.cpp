...
void cSPH::InitScene()
{
	...
	//  app upd   preserve..
	App::camPosLag = scn.camPos;
	App::camRotLag = scn.camRot;
...
void cSPH::LoadScenes()
{
	scenes.clear();
	TiXmlDocument file;
	if (!file.LoadFile("Scenes.xml"))
	{
		printf("\nError: Can't load Scenes.xml. %s", file.ErrorDesc());
	}
	...
	{
		s = root->FirstChildElement("Scene");
		if (!s)
			printf("Warning:  No <Scene> in xml.\n");
		opt = root->FirstChildElement("Options");
		if (!opt)
			printf("Warning:  No <Options> in xml.\n");
	}
	...
	while (s)
	{
		if (s->Attribute("default") || s->Attribute("def"))
			curScene = i + 1;
...
void cSPH::LoadOptions()
{
	...
	if (opt)
	{
		a = opt->Attribute("Windowed");
		if (a)
			App::bWindowed = toInt(a) > 0;
		a = opt->Attribute("WSizeX");
		if (a)
			App::WSizeX = toInt(a);
		a = opt->Attribute("WSizeY");
		if (a)
			App::WSizeY = toInt(a);
		a = opt->Attribute("VSyncOff");
		if (a)
			App::bVsyncOff = toInt(a) > 0;

		a = opt->Attribute("timAvgCnt");
		if (a)
			App::timAvgCnt = toInt(a);
		a = opt->Attribute("barsScale");
		if (a)
			App::barsScale = toInt(a);

		a = opt->Attribute("showInfo");
		if (a)
			App::bShowInfo = toInt(a) > 0;
	}
	//...
}